...
**Optional repository governance.** The package also publishes a separate binary, `eval-quality-gates`. It is independent of the Behavioral Evaluation Contract flow and is used for deterministic repository policies such as dependency direction, documentation claims, licences, and package boundaries. The configuration file carries only the gates you have adopted, and a gate you invoke with no section for it refuses by name with no fallback to this package's own values. [Repository Governance Gates](https://bmad-code-org.github.io/bmad-eval-quality/how-to/run-the-gates-on-your-repository/) is the page for it.
