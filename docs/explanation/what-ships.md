...
Also published: twelve JSON Schema documents under `eval-quality/schemas/*`, a twenty-two-contract development corpus under `eval-quality/corpus/dev/`, five reference adapters at `eval-quality/adapters`, and a port conformance suite at `eval-quality/conformance`.
