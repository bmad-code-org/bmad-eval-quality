...
The evidence artifact keeps each detailed oracle outcome with its `trialIndex` and publishes the reduced per-probe outcome separately. Each reduction records the policy's `catchThreshold`, so its caught decision can be verified from its counts. Contract strength and dominance use the reduced result after checking it against the detailed trial evidence.
