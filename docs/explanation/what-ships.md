...
The evidence artifact keeps each detailed oracle outcome with its `trialIndex` and publishes the reduced per-probe outcome separately. Each reduction retains the selected `trialVotes` and the policy's `catchThreshold`, so its counts, invalidations, and caught decision can be recomputed exactly. Contract strength and dominance use the reduced result after checking it against the detailed trial evidence.
