...
`seal` is what keeps the two arms comparable. The brief it emits carries eleven top-level fields: `schemaVersion`, `parentDigest`, `revisionCount`, `contractDigest`, `behaviors`, `permittedInterfaces`, `scopedResources`, `budgets`, `safetyLimits`, `probeStepBound`, and `directions`, which is one prose direction per oracle. Each permitted interface narrows to its `logicalId` and `kind`. The withholding is done by the brief's shape: the oracle checks, the interaction plan, the reference sets, and the test data have nowhere to go in it, so an evaluator reading a brief cannot read the answers off the contract.
