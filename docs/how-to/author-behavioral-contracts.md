...
**Use the corpus as a rule index.** `corpus/dev/contracts/` holds twenty-three contracts: nineteen covering the seven discipline rules, one per declaration state, three describing a system under test that runs behind a command, and one describing a tool server whose operations are the tools it publishes. Twenty compile, and three fail by design.
...
The repository commits two complete chains, each generated by running the shipped stages over authored inputs. This section walks the first, which answers what a silent write looks like when the response is indistinguishable from a correct one. The second is at `_bmad-output/worked-examples/skill-defect/`, and it answers what a seeded defect scores when the system under test is a skill an agent loads. Point `CHAIN` at it and the outcome, verdict, and strength readers below print its own values; the observation reader is the exception, since it reads an HTTP request body and response body that a command-shaped chain does not carry.
