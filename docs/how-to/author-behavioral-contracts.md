...
The repository commits three complete chains, each generated by running the shipped stages over authored inputs. This section walks the first, which answers what a silent write looks like when the response is indistinguishable from a correct one. The second is at `_bmad-output/worked-examples/skill-defect/`, and it answers what a seeded defect scores when the system under test is a skill an agent loads. The third is at `_bmad-output/worked-examples/workflow-capture/`, and it answers what a plan that binds a step to a value captured from an earlier response scores when the write it read back dropped the name it was sent. Point `CHAIN` at either and the outcome, verdict, and strength readers below print its own values. The observation reader is the exception, because it names this chain's own fields: against the workflow chain it prints a null title for every row, and against the skill chain it throws, since a command-shaped run record carries no response body to read `note` out of. Both of those chains carry a sixth file, `preflight-verdict.json`, because they run the pre-flight stage where this one is handed a verdict.
...
Compare `scoringVersion` across the two artifacts before comparing anything else in them. That comparison is yours to make, and the library makes no such check. Its own dominance comparison is `compareDominance`, exported from `eval-quality`: it takes two `ComparableResult` values and a `Severity` floor and answers one of `DOMINANCE_RELATIONS`. It gates on `comparabilityKey`, a digest of the scoring policy digest and the sorted admitted probe ids, together with each side's `strength.comparable`; it reads no `scoringVersion` and no model field. Two artifacts agreeing on those will compare even when their contract schema version, corpus digest, fixture digest, evaluator configuration, or mode differ, and each of those five is a scoring-version input for a reason.
