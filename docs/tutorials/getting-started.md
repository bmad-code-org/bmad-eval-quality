...
The artifact itself is one line of JSON with the keys in sorted order. That exact payload is what a digest is computed over, so two machines agree on what the contract is.
