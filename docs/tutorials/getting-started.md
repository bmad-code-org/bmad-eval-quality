...
An **eval contract** is a JSON document that says what a system is supposed to do, written so an automated check can resolve it. The package ships twenty-three of them, so you do not have to write one yet.
