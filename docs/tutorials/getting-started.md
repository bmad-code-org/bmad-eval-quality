...
An **eval contract** is a JSON document that says what a system is supposed to do, written so an automated check can resolve it. Its full name is a Behavioral Evaluation Contract, and people shorten that to BEC. The package ships twenty-four of them, so you do not have to write one yet.
