...
`compareDominance` is AD-7's four-valued relation over two scored results. It takes two `ComparableResult` values and a `Severity` floor and answers one of `DOMINANCE_RELATIONS`: `a-dominates-b`, `b-dominates-a`, `equivalent`, or `incomparable`. `ComparableResult`, `DominanceRelationValue`, and `Severity` ship as type-only exports beside it. A comparable result includes `outcomes`, `trials`, and `reducedProbeOutcomes`; each reduction retains the selected `trialVotes` and `catchThreshold` needed for exact recomputation. The comparison returns `incomparable` when the reduction contradicts its votes, counts, invalidated attempts, or detailed trial evidence. It re-derives no strength vector and reads no port, corpus, or clock.
