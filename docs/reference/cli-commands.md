...
The reasons that rung would have recorded in `verdictBasis` go to stderr instead, one `eval-quality: invalid: <reason>` line per condition that fired, so an exit `3` always names its cause.
