...
| **Dominance** | How two contract-strength vectors compare under `compareDominance`. Four-valued: `a-dominates-b`, `b-dominates-a`, `equivalent`, or `incomparable`. Gated on matching `comparabilityKey` and `comparable: true` on both sides, with a severity-floor override preventing any contract that missed a behavior at or above the floor from dominating one that caught it. |
