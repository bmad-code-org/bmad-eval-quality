...
| **Trial set** | Several runs of one probe. They reduce to one result per probe before any rate is computed, because a pass-if-any reading is the retry anti-pattern. Repeat `--record` on the `score` command to supply the set. |
...
| **Dominance** | How two contract-strength vectors compare under `compareDominance`. Four-valued: `a-dominates-b`, `b-dominates-a`, `equivalent`, or `incomparable`. Gated on matching `comparabilityKey`, `comparable: true`, and reduction consistency (`reductionDetailsAgree`) on both sides, with a severity-floor override preventing any contract that missed a behavior at or above the floor from dominating one that caught it. |
| **Comparability key** | The gate for dominance comparison: a SHA-256 digest of the scoring policy digest and the sorted admitted probe identifiers. Deliberately narrower than `scoringVersion` so that contract strength can be compared across evaluation runs sharing the same policy and probe set. |
...
| **Scoring version** | The full experiment identity of a scored result: the digest of six inputs, the contract schema version, the corpus digest, the fixture digest, the evaluator configuration digest, the scoring policy digest, and the run mode. Callers compare scoring versions to ensure experimental controls held constant across runs. |
...
| **Interface kind** | What sort of surface an interface is. `compile` accepts `api`, a system behind an HTTP API, `cli`, a system behind a command, and `mcp`, an MCP tool server. The vocabulary also names `web`, and a contract declaring it is rejected with `unsupported-interface-kind`. |
...
| **Planted defect (mutation)** | The one intentional change, whose expected failure mode is known in advance. You make the edit yourself: no command in the package mutates anything. A weakened prompt, removed context, a dropped validation step, an altered tool result. It has to be an artifact you hold: the probe's `controlled-mutation` qualification route requires a `targetArtifact` naming what changed and `rollbackVerified` proving it was restored, and vendor model weights fill neither field. |
| **`mutationOperator`** | Free text on a `controlled-mutation` qualification record, naming the edit its author made in their own words, such as `store-write-deletion`. No code reads the value and there is no vocabulary of operators to draw from: the field records what a human did. |
