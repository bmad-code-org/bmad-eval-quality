...
async function main() {
  if (process.platform === 'win32') {
    console.error('Error: The docs build pipeline does not support Windows directly.');
...
function generateLlmsFullTxt(docsDir, outputDir) {
  ...
  console.log(`    Processed ${fileCount} files, ${result.length.toLocaleString()} chars (~${tokenEstimate} tokens)`);
}
...
function validateLlmSize(content) {
  ...
  if (charCount > LLM_MAX_CHARS) {
    ...
    console.warn(`    WARNING: Approaching ${LLM_WARN_CHARS.toLocaleString()} char limit`);
  }
}
