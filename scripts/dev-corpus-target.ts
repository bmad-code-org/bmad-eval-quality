...
/**
 * The example's seed, named here because three of the twenty-two do not compile
...
Twenty-two contracts and one compiled-and-sealed pair, published so an adopter can read real input to
...
- \`contracts/<contractId>.json\`: twenty-two contracts. Nineteen are one per discipline rule in each
  declaration state, two describe a system under test that runs behind a command, and one describes a
  tool server whose operations are the tools it publishes.
  Three fail compilation by design; those ship as authored input, and \`index.json\` records the
  failure code each one raises. The other nineteen are published only after this package's own
