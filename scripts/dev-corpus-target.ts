...
/**
 * The example's seed, named here because three of the twenty-three do not compile
...
Twenty-three contracts and one compiled-and-sealed pair, published so an adopter can read real input to
...
- \`contracts/<contractId>.json\`: twenty-three contracts. Nineteen are one per discipline rule in
  each declaration state, three describe a system under test that runs behind a command, and one
  describes a tool server whose operations are the tools it publishes. One of the command contracts
  holds a skill responsible for a decision an agent carried out: it declares two behaviors with one
  oracle each, which is what pairs a probe with the oracle discharging the behavior it breaks, and it
  is the contract one of the two committed end-to-end chains scores a seeded defect against.
  Three fail compilation by design; those ship as authored input, and \`index.json\` records the
  failure code each one raises. The other twenty are published only after this package's own
...
\`scripts/worked-example-target.ts\` and \`scripts/skill-example-target.ts\` and exercised there
through \`ingest\`/\`score\`/\`emit\`. Two chains are committed and the run record is the only one of
the three each of them ships: \`spike-worked-example/sealed-run-record.json\` and
\`worked-examples/skill-defect/sealed-run-record.json\`. The isolation manifest and the evaluator
configuration exist only as the authored values each build passes to \`ingest\`. This directory ships
only the compile-and-seal pair, scoped to what a corpus of contracts needs.
`
