// The byte-exact drift check (AD-13) for AD-33's decision table: rebuilds
// through the same pure builder the writer uses and compares against the
// committed `docs/ad33-outcome-decision.generated.md`, so the published table
// cannot claim a decision the procedure no longer makes.
//
// It never rewrites the file, per `check-schemas.ts:8-9`: a check that can
// repair what it checks is not a gate. Regeneration is
// `npm run generate:ad33-table`.
//
// No orphan check: `docs/` now holds two generated tables, and this one names
// the file it owns through the shared target constant, so a third file
// appearing there is a documentation decision, outside this check's remit.
//
// Usage:
//   npm run check:ad33-table

// Run by `node` directly: type stripping erases types only, so no TypeScript
// enum, namespace, parameter property, or non-type re-export may appear here
// or in anything it imports. That binds both score modules and the fixture
// module.
import { readFile } from 'node:fs/promises'
import {
	CORROBORATION_RULES,
	INVALIDATING_CONDITIONS,
	OUTCOME_RULES,
	resolveOutcome,
	WAIVER_RULES,
} from '../src/core/score/outcome.ts'
import { outcomeDecisionTable } from '../src/core/score/outcome-table.ts'
import {
	fixtureCases,
	infeasiblePairs,
	STRUCTURAL_CONSTRAINTS,
} from '../tests/score/fixtures/outcome-inputs.ts'
import {
	AD33_TABLE_FRONTMATTER,
	AD33_TABLE_PATH,
	AD33_TABLE_TARGET,
} from './ad33-table-target.ts'

const cases = fixtureCases()

// Guarded: unguarded, the builder's diagnosis surfaces as an unhandled
// rejection and a Node stack.
let rebuilt: Buffer
try {
	rebuilt = Buffer.from(
		`${AD33_TABLE_FRONTMATTER}${outcomeDecisionTable(
			cases.map(resolveOutcome),
			{
				constraints: STRUCTURAL_CONSTRAINTS,
				infeasiblePairs: infeasiblePairs(),
			},
		)}`,
		'utf8',
	)
} catch (error) {
	console.error(
		`check-ad33-table: the builder failed: ${
			error instanceof Error ? error.message : String(error)
		}`,
	)
	process.exit(1)
}

let committed: Buffer
try {
	committed = await readFile(AD33_TABLE_TARGET)
} catch {
	console.error(
		`${AD33_TABLE_PATH}: missing; run \`npm run generate:ad33-table\``,
	)
	process.exit(1)
}

if (!committed.equals(rebuilt)) {
	// First differing byte offset with a window either side, so the drift is
	// locatable without a manual diff.
	const bound = Math.min(committed.length, rebuilt.length)
	let offset = 0
	while (offset < bound && committed[offset] === rebuilt[offset]) offset++
	const windowOf = (buffer: Buffer): string =>
		JSON.stringify(
			buffer.subarray(Math.max(0, offset - 20), offset + 20).toString('utf8'),
		)
	console.error(
		`${AD33_TABLE_PATH}: drift at byte offset ${offset} ` +
			`(committed ${committed.length} bytes, rebuilt ${rebuilt.length} bytes)\n` +
			`  committed: ${windowOf(committed)}\n` +
			`  rebuilt:   ${windowOf(rebuilt)}`,
	)
	process.exit(1)
}

const ruleCount =
	OUTCOME_RULES.length + WAIVER_RULES.length + CORROBORATION_RULES.length
console.log(
	`check-ad33-table: the committed AD-33 table matches the builder byte for byte (${ruleCount} rules, ${INVALIDATING_CONDITIONS.length} conditions, ${cases.length} cases)`,
)
