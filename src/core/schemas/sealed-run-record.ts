...
export const Observation = z.strictObject({
	...
	provenance: z
		...
		.describe(
			"AD-23, defined by the observation's role in the run. `evaluator-chosen` is an action the evaluation itself performed on the probed interface, whether an agent selected it or a scripted plan did, so a deterministic interaction plan that is the evaluation's own action records every step as `evaluator-chosen`. `baseline` is a harness baseline or a fixture set-up call the evaluation did not perform, and AD-40's witness match excludes it: only `evaluator-chosen` observations can exercise or witness a probe. The distinction is the one the product's central finding rests on: what a sealed evaluator detects beyond the pre-canned baseline. It is a field of the observation, and a finding carries no provenance of its own.",
		),
...
export const SealedRunRecord = z
	.strictObject({
		...
		trialIndex: z
			...
			.min(1)
			.describe(
				"Which trial this record is. AD-24 excludes the trial index from the Evaluator Configuration \"so trials pool into one scoring version\", which requires it somewhere else, and a Sealed Run Record is the only artifact carrying exactly one trial. One-based and unique within the trial set a caller presents. AD-6's aggregate of trial count, invalidated attempts, and each attempt's reason is the Evidence Artifact's, computed by `score` over that set. The reducer retains this value as an invalidated attempt's identity, so presentation order cannot relabel an attempt.",
			),
