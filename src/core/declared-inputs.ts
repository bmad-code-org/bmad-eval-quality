 ...
 * pointer grammar and the loops that walk it now run over nine. A record with
 * no key for a channel carries nothing on it, which is the same answer as the
 * `null` those two shapes already spell for a channel nothing was sent on. The
 * ninth key lands with the sealed run record's own breaking bump, and this is
 * what keeps a loop over the vocabulary total until it does.
 */
export const channelEntryOf = <Value>(
	record: Readonly<Record<string, Value | null>>,
	channel: InputChannelName,
): Value | null => record[channel] ?? null
