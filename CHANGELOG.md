...
- **BREAKING** The probe's `schemaVersion` is 4. A manifestation witness's `inputs` gains the same
  third arm, so a seeded defect against an MCP tool server is declarable. Every version-3 probe
  parses with no edit, and no reader in this version compares the probe's stamp against a constant.
- A tool call's evidence is confined at compile time as well as at qualification. An oracle pointer
  at `response-headers`, `exit-code`, a stream, or a written file on an `mcp` operation is
  `unreachable-check-evidence`: a tool call carries its structured result on `response-body` and its
  error flag on `response-status` and fills nothing else. A witness leg on one carries the same
  three channels.
- A transport identity is compared inside its own kind. A tool named `notes` and an executable named
  `notes` are different things on different servers, so `duplicate-operation-signature` no longer
  reports the pair, and `resolveHomeOperation` stops sweeping an `mcp` interface into the
  api-shaped comparison.
