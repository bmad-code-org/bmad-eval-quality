...
> **Qualified 2026-07-29 by [ADR-008](../../architecture/architecture-eval-quality-2026-07-29/ADR-008-compile-half-owed-to-calibration.md).** The rates are real and the decision to build stands, but this paragraph states the measurement more strongly than its own record does, and round 3 was right that a product built to make evaluation quality falsifiable should not do that. Three qualifications, all verified against the experiment's own outputs. The 0.33-to-1.00 figure is pooled over three cases with separation on **two**, since both arms detected the third in three of three repetitions. Both separating cases carry a recorded measurement-layer confound. And the block's own preregistered decision was **CONTRACT-DISCIPLINE NOT SUPPORTED**: four of five gates passed, and the one that failed did so on a single unreplicated clean control where the preregistration asked for one per arm per case. The characterisation above — that the failed gate traced to a harness defect rather than to a contract or a system — is the experiment record's own reading and is defensible. What changes is that the qualifications now travel with the number wherever it justifies a decision, and that a catch rate calibrated against this block names the block-2 replication as a dependency rather than assuming it.

Current decisions:

1. `eval-quality` is the single name for the agents, rules, methodology, governance, and any evaluator pack.
2. ~~Use an existing open-source engine for execution, trace ingestion, standard assertions, repeated runs, reports, and CI integration.~~ **Corrected 2026-07-29 by ADR-004: eval-quality executes nothing and no engine supplies the runner.** `agentevals-dev/agentevals` scores pre-recorded OpenTelemetry traces, does not execute agents, and is Python-only with no npm distribution, so it cannot fill this role. The caller runs the agent against a sealed brief and returns a run record for ingestion. v0 depends on no engine; engine integration becomes a v0.1 adapter behind a port. eval-quality does not reimplement any engine's run CLI, and it does ship its own command surface over its own capabilities (VFR-8).
...
**Architecture, 2026-07-29.** The formal architecture pass is
[ARCHITECTURE-SPINE.md](../../architecture/architecture-eval-quality-2026-07-29/ARCHITECTURE-SPINE.md),
recorded as [ADR-003](../../architecture/architecture-eval-quality-2026-07-29/ADR-003-measurement-mechanics.md),
[ADR-004](../../architecture/architecture-eval-quality-2026-07-29/ADR-004-execution-boundary.md), and
[ADR-005](../../architecture/architecture-eval-quality-2026-07-29/ADR-005-review-round-corrections.md).
It closes everything VFR-7 deferred and corrects this document in four places, each marked inline
below: the engine's role in item 2 and VFR-6, the aggregate score and outcome-state spelling in VFR-7,
the executing engine in VFR-8, and the Node floor. Where this PRD and the spine disagree on a
mechanic, the spine governs; where they disagree on product direction, this PRD governs.

ADR-005 records a second round of corrections after four independent external reviews returned 79
findings, and amends this document in five further places, also marked inline: how an outcome state is
resolved and the omission of `bypassed` from the behavioural failures in VFR-7, the rate-based strength
vector in VFR-7, the verdict table, severity routing, and exit codes in VFR-5, the limits of holdout
enforcement in VFR-7, and the runtime floor in VFR-8. Two of those are product-direction changes rather
than mechanics, so they are called out here as well: **severity now routes the gate verdict**, which
this document's dual-gate language never provided for, and **CONCERNS exits zero** unless the caller
passes `--strict`, which changes what wiring eval-quality into a release gate does by default.

[ADR-007](../../architecture/architecture-eval-quality-2026-07-29/ADR-007-compile-score-split.md) records a
third round, after four further reviewers returned 50 findings against spine revision 3 and three of them
said do not build. It carries one product-direction change large enough to state at the top of this document:
**VFR-2's compile stage is the only part of v0 that is build-ready, and VFR-7's scoring stage is not** — *the
first half of that sentence was withdrawn the same day by ADR-008; see the paragraph below.* Five
of the round's ten root causes were specification gaps in `compile` and are closed; three of the six in
`score` were not specification gaps at all. The worst is that **the catch rate this PRD treats as the headline
measurement was 1.00 by construction** — no finding was ever matched to the defect its probe seeded, so
non-detection could not be recorded. The spine's *Owed to the reference implementation* section records the
open score-side defects, and no epic builds against them until a reference implementation of the scoring
decision procedures closes them. VFR-7's product intent is unchanged; its readiness claim is withdrawn.

[ADR-008](../../architecture/architecture-eval-quality-2026-07-29/ADR-008-compile-half-owed-to-calibration.md)
records a fourth round, which put four reviewers against spine revision 4 and returned four verdicts of do
not build. **It withdraws the remaining readiness claim: no part of v0 is build-ready, including VFR-2's
compile stage.** Three reviewers independently falsified the sentence ADR-007 relied on. Every contract
behind the 0.33-to-1.00 measurement declares an MCP tool interface that v0 rejects at compile time, so the
compile half cannot ingest its own calibration corpus. The oracle content that *caused* the measured effect
— preconditions on what the test data and the evidence set must contain, and clauses stating what
harmless-looking evidence does not discharge — has no field in the structured contract grammar and no seat
on the sealed brief. And a compile-stage requirement was published only from an artifact that the scoring
half alone can produce. The spine's new *Owed to the calibration re-run* section records the open
compile-side defects, and the gate on them is an experiment rather than a review: generate oracle prose
from the structure and re-run a frozen evaluator arm against it. Two further corrections belong at the top
of this document rather than in a footnote, because both are claims this PRD makes elsewhere. **The
measured effect's own block decision was CONTRACT-DISCIPLINE NOT SUPPORTED** — see the correction at line
14. And the assertion that the original instrument used blinded adjudication after sealing is false; it
used a mechanical containment rule over quoted evidence, and blinded adjudication belongs to an earlier
round on a different system.
...
Before implementation begins, the contract author converts the product spec into a versioned Eval Contract containing behavior goals, requirement and risk identifiers, oracles and rubrics, permitted black-box interfaces, test-data and cleanup rules, safety limits, budgets, and evidence requirements. The contract contains no prescribed action sequence. **Refined 2026-07-29 by ADR-007:** "no prescribed action sequence" is now a checkable boundary rather than a phrase, because taken literally it rejected this project's own canonical contract while supplying no predicate a compiler could run. A contract may declare the witness relations a behaviour needs — read-after-write, for instance, since "did the write persist" inherently means "read after write" — and may not declare an ordered path, an exhaustive trace, or any step whose ordering the behaviour's meaning does not require. **Corrected 2026-07-29 by ADR-008:** the boundary was said to be calibrated against the experiments' plain, disciplined, and **scripted** contract arms. There is no scripted arm — the block has two, and a search of the entire experiment record returns no scripted contract, run, or fixture. The prohibition is sound by design and the run records support the reasoning behind it, but it was presented as an empirical result and is not one. The enforceable boundary is now a published graph predicate over the declared interaction plan, bounding width as well as depth, calibrated with accept fixtures from the two real arms and reject fixtures authored deliberately, one per rejected shape.

> **Readiness withdrawn 2026-07-29 by [ADR-008](../../architecture/architecture-eval-quality-2026-07-29/ADR-008-compile-half-owed-to-calibration.md).** ADR-007 named this capability the one build-ready part of v0. Round 3 falsified that from three directions and no epic builds it either. Every contract behind the measured effect declares an MCP interface v0 rejects at compile time, so this capability cannot ingest its own calibration corpus and is calibrated against API-shaped transcriptions instead. The oracle content that produced the measured effect has no field in the structured direction and no seat on the sealed brief, which is the open defect that gates the capability. And a compile-stage requirement was published only from an artifact the scoring half alone can regenerate, now corrected. The spine's *Owed to the calibration re-run* section records all three; the capability's product intent is unchanged.
...
Oracles are written in a small shared operator vocabulary applied uniformly to call inputs and parsed outputs: equality, containment, existence and absence, regex, set membership, ordering, structural deep equality, count tolerances, and shape checks such as schema-plus-required-keys. **Extended 2026-07-29 by ADR-006 and ADR-007:** the vocabulary also carries the connectives, the collection quantifiers `for-all` and `for-any`, and one bounded relational operator `covers-by-key`. The quantifiers exist because "every record in the response is complete" is otherwise inexpressible and only the spot-check the discipline forbids can be written; `covers-by-key` exists because reconciling what should be present against what was returned is otherwise unwritable, which left the omission rule satisfiable by a response that repeats one record three times and omits two. A quantifier over an empty collection resolves **false**, not vacuously true, because a discipline rule that certifies itself over zero records is worse than an absent one. A strict mode fails on undeclared inputs, catching behavior the interface silently ignores. Zero-action negative probes express what the system must not do at all. Rubrics follow fixed authoring rules: anchored scales, named failure-mode penalties, bounded length, and no questions a sealed evaluator cannot answer from observable evidence. An oracle may abstain with an explicit unsure result rather than being forced into a binary call it cannot support. Abstentions surface as their own signal in the evidence, and under VFR-7 an abstaining required check cannot produce PASS.
...
> **Amended 2026-07-29 by [ADR-005](../../architecture/architecture-eval-quality-2026-07-29/ADR-005-review-round-corrections.md) Decisions 2 and 4.** Verdict derivation is a total table rather than a precedence order ending in "PASS otherwise", because that fallthrough gave a green gate to two conditions this section requires be honoured: an ingested evaluator recommendation of FAIL, and an evidence set marked unavailable. Severity now routes between FAIL and CONCERNS on the closed vocabulary `low`, `material`, `critical` against a policy-named floor — a gate that treats a missed critical behaviour and a missed cosmetic one identically is as unbacked as one that invents weights. A fourth verdict, WAIVED, keeps a mostly-waived run distinguishable from one that caught everything. Exit codes: PASS, WAIVED, and CONCERNS exit zero with `--strict` promoting CONCERNS to one, since exit one is indistinguishable from a crash and a fatal advisory gate attracts `|| true`, which silently discards FAIL along with it; FAIL is two, an invalid run three, a structural compile failure four, and a thrown fault five, so a command producing no verdict never exits inside the verdict range.

#### VFR-6: Reuse an existing engine

> **Corrected 2026-07-29 by [ADR-004](../../architecture/architecture-eval-quality-2026-07-29/ADR-004-execution-boundary.md).** The requirement as written below does not hold, because the engine named for the role executes nothing and cannot be imported from TypeScript. The corrected requirement: **eval-quality executes nothing.** It never spawns a process, calls a model, drives a system under test, or invokes a judge. It compiles, seals, ingests, runs pre-flight, scores, and emits. The caller brings its own agent, runs it against the sealed brief, and returns a run record. Repeated trials become the caller's loop over sealed briefs; reporting is the evidence artifact plus exit codes. No engine is a v0 dependency.
...
A contract is scored before it is trusted: seed a known defect class behind the contract, run an evaluator against it, and report which oracles caught it. Two probe classes seed defects: defect probes, where the behavior is wrong, and gameability probes, where the behavior looks compliant while dodging the oracle's intent. A strong contract rejects both. Eval Contract strength scores are comparative diagnostics, never absolute verdicts, and per-oracle outcomes are the primary output. ~~An aggregate score is a summary of them.~~ **Corrected 2026-07-29 by [ADR-003](../../architecture/architecture-eval-quality-2026-07-29/ADR-003-measurement-mechanics.md): there is no aggregate score.** Strength is an outcome-state vector compared by dominance, with no weights and no composite number; pairs that neither dominate are reported `incomparable`. The dual aggregate-plus-per-clause gate named in decision item 9 therefore reduces to its per-clause half.

> **Amended 2026-07-29 by [ADR-005](../../architecture/architecture-eval-quality-2026-07-29/ADR-005-review-round-corrections.md) Decisions 1 and 3.** Two changes to how this is measured. Outcome state resolves from the sealed evaluator's ingested findings, not from the contract's own machine-evaluated assertion — which is what the experiments measured, and what "report which oracles caught it" above already implies. And the compared vector holds catch *rates* over unique qualified probe identifiers across a declared minimum trial count, defaulting to three, because raw counts let a contract scored over more trials dominate a better contract scored over fewer. Component-wise equality now reports `equivalent` rather than `incomparable`.

> **Readiness withdrawn 2026-07-29 by [ADR-007](../../architecture/architecture-eval-quality-2026-07-29/ADR-007-compile-score-split.md).** This capability's product intent stands and its mechanics do not yet. Round 2 of review established that **"report which oracles caught it" was not computable**: nothing matched a finding to the defect its probe seeded, so an oracle that correctly confirmed unrelated correct behaviour and an oracle that failed to detect the seeded defect were indistinguishable from every input the scorer read, and `missed` was unreachable. **The catch rate was 1.00 by construction.** Spine AD-40 supplies the missing input — every probe declares a machine-readable defect signature, and a probe counts as detected only when a mapping identifies its seeded defect among the findings cited against it — and deliberately does not yet claim that mapping can be mechanical, since the instrument behind the measured effect used blinded adjudication after sealing. **Corrected 2026-07-29 by ADR-008: it did not.** That block used a mechanical containment rule over quoted evidence with the detection field written by the orchestrator; blinded adjudication belongs to an earlier round on a different system. Two reviewers built the mapping and ran it, and one reproduced that earlier round's adjudication exactly on the real trace, so the mapping now commits to a deterministic witness match — recorded as unvalidated until a run preserves the evidence a matcher needs, since the existing sealed records carry paraphrase rather than quotation. The signature itself was also rerooted, from contract-declared step identifiers, which the probe author is forbidden to see, to transport identity.
>
> Three further score-side defects remain open and are recorded in the spine's *Owed to the reference implementation* section rather than resolved in prose: repeated trials have no reducer and no stage consumes more than one run record, so the three-trial minimum above is currently unreachable; observation selection is ambiguous, so two conforming scorers can disagree on identical evidence; and a defect the evaluator finds outside any oracle — SM-D4, the differentiating result of the whole experiment — moves nothing. No epic builds this capability until a reference implementation of the scoring decision procedures closes them.

**Outcome states.** Every required oracle check in a scored run resolves to exactly one of `caught`, `missed`, `passed-clean-control`, `false-positive`, `abstained`, `bypassed`, `oracle-error`, `judge-error`, `infrastructure-error`, or `not-applicable`. Corrected 2026-07-29 by ADR-003: values are kebab-case, matching the eight schemas under `experiments/hypothesis-validation/schemas/` that validated the H0 population, and the set is closed at ten. (An earlier version of this correction said those schemas "carried 19 scored runs"; that was false and is itself corrected here — the 19 repetitions are Phase 2 block 1, whose raw evidence is held privately.) The state carries the result, so "the check reported" is never sufficient on its own:

- A defect or gameability probe counts as detected only when its mapped oracle returns `caught`.
- `missed`, `abstained`, `bypassed`, `false-positive`, or any absent required check prevents PASS as a behavioural failure.
- `oracle-error`, `judge-error`, `infrastructure-error`, and a failed environment pre-flight are not behavioral results. They invalidate the run and are re-executed rather than scored. No state both prevents PASS and invalidates.
- `bypassed` records a probe whose oracle was evaded rather than answered, and never counts as a catch.
- `not-applicable` is legal only against a compiled waiver carrying a named rule, an explicit rationale, a machine-checkable context condition, a recorded approval, and an expiry, and is excluded from every count.
- Clean controls must return `passed-clean-control` with zero `false-positive` outcomes.
- A run in which nothing resolved is invalid rather than a pass.

> **Amended 2026-07-29 by [ADR-005](../../architecture/architecture-eval-quality-2026-07-29/ADR-005-review-round-corrections.md) Decisions 1 and 2.** Three corrections. `bypassed` is added to the behavioural failures above: this list omitted it while the same paragraph says it "never counts as a catch", so a gameability probe the contract accepted would have passed the gate. Outcome state is assigned from the evaluator's findings mapped to oracles, through a published total table over finding, check resolution, probe class, control status, and waiver state, so no combination is left to an implementer. And every check declares a polarity, without which an expression resolving false reads as `caught` to one implementer and `missed` to another.

> **Further amended 2026-07-29 after hand-authoring the first contract.** The set is twelve, not ten. Two states were missing and both are ordinary rather than exotic. `confirmed` records an oracle that correctly established correct behaviour on a probe whose defect lies elsewhere — the common case on any defect probe, since most oracles correctly confirm the behaviours the seeded defect does not touch, and it was being written as `passed-clean-control`, which is reserved for known-clean controls. `unreached` records an oracle whose declared observations the evaluator never produced, which is an evidence condition rather than a claim about the system. A run record must also now carry one disposition per required oracle, because silence is ambiguous between "judged fine and not filed", "declined to judge", and "never got to it", and those resolve to three different states. See `spike-worked-example/FINDINGS.md` in the architecture folder.
...
> **Amended 2026-07-29 by [ADR-005](../../architecture/architecture-eval-quality-2026-07-29/ADR-005-review-round-corrections.md) Decision 6.** The prohibition stands as policy; what changes is who enforces it. A stateless package cannot detect that unchanged probe content was revealed to its author and reused, and a caller who starts a fresh lineage resets the revision count. eval-quality validates a presented lineage chain and emits compliance evidence, recording the remediation count as caller-attested. Enforcement across evaluations is the adopting team's governance. This is stated rather than implied so nobody relies on a guarantee the boundary cannot provide.
...
Score formulas, weighting, thresholds, corpus rotation, probe and result schemas, and the aggregation algorithm are implementation detail for the architecture stage rather than product invariants. **Closed 2026-07-29** in [ADR-003](../../architecture/architecture-eval-quality-2026-07-29/ADR-003-measurement-mechanics.md) and enforceably in [ARCHITECTURE-SPINE.md](../../architecture/architecture-eval-quality-2026-07-29/ARCHITECTURE-SPINE.md): dominance over outcome-state vectors, weighting refused, verdict by rule precedence rather than threshold, additive corpus rotation with caller-resolved sealed sets, and a closed eleven-artifact set.
...
Command-surface invariants: non-interactive by default so it is callable headlessly, machine-readable output as the default format, an exit code that reflects the gate verdict, and the same versioned evidence artifact whichever surface produced it. ~~The engine still executes the agent (VFR-6);~~ **corrected 2026-07-29 by ADR-004: the caller executes the agent and nothing in eval-quality does.** eval-quality's commands compile contracts, seal briefs, ingest run records, run the pre-flight, score contract strength, and emit evidence. Inputs arrive by path or stdin, outputs go to a run-scoped directory with machine-readable stdout and diagnostics on stderr, and no command mutates an input in place.

Packaging follows the terms already recorded under Archived Developer-Product Requirements, adopted here as current: TypeScript, Node >= 22.20.0 (corrected 2026-07-29 from Node >= 20, which reached end of life in April 2026; development and CI run Node 24, the Active LTS; the floor names a patch rather than the major line because a `>=22` floor paired with the 22-line type surface lets code typecheck against APIs Node 22.0.0 does not have), ESM, Apache-2.0, unscoped npm name `eval-quality`, permissively licensed dependencies only. **Added 2026-07-29 by ADR-007, corrected by ADR-008:** the build stack pins npm **11.18.0 exactly** and asserts that exact version in every CI job, because Node 22.20.0 bundles npm 10.9.3 while this repository's `min-release-age` and `allow-git` supply-chain controls require npm 11.10.0 and 11.15.0 respectively — so at the declared runtime floor both keys were read and silently ignored, and a package published one day earlier installed cleanly under a seven-day policy. ADR-007's fix, a `>=11.15.0` range with a resolution canary, failed open twice over: the range admits npm 12, whose engines exclude the declared Node floor, and `min-release-age` filters fresh resolution while CI runs `npm ci`, so a young package already in the lockfile installs cleanly under an active policy. CI therefore audits the resolved lockfile's publication ages before installing, and its canaries run the ordinary job path rather than a resolution shortcut. This is a build-toolchain floor and does not raise the runtime floor for adopters. Pre-1.0 versioning applies, and breaking changes are called out on every release across the whole caller-facing surface: the contract schema, the sealed brief, the sealed run record, the isolation manifest, the evaluator configuration, the evidence artifact, the fault-code registry, and the CLI command, flag, and exit-code contract.
...
- **SM-D4:** At least one defect is found through an evaluator-chosen action absent from the pre-canned baseline. *(Flagged 2026-07-29 by ADR-007: this metric's evidence is retained by the architecture but consumed by nothing — such a finding maps to no oracle, enters no rate, and moves no verdict, so the differentiating result of the whole experiment currently produces a line in an array and exit code zero. Giving it a verdict rung is an open item in the spine's Owed to the reference implementation section.)*
...
- TypeScript, Node >= 20 (superseded 2026-07-29: the current floor is Node >= 22, per VFR-8), ESM (`"type": "module"`).
