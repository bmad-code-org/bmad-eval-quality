...
**Approach:** Build `src/core/ingest/` as a pure function over already-parsed artifacts, returning the validated observations, the ladder inputs it can fill today, and every condition it detected. Each condition carries the payload its consumer needs, which for the quotation audit is the consumer's own declared shape and for the isolation family is the list shape the ladder field is about to widen to. Eight of the eleven conditions have no rung on either shipped ladder, so they are recorded with a named owner instead of being forced into a rung that means something else.
...
**The eleven conditions. Nine bullets below quote a field description and a tenth carries the framing; the third and fifth feed one condition between them, and the two absent cases have no bullet because no schema can describe an absent artifact. `duplicate-record-identifier` has no bullet either, and for a different reason: no description states it, because `SealedRunRecord` refines uniqueness on `observations.sequence` alone and says nothing about the other two arrays. Seven of the eleven were known at approval; the disposition citations and the two evaluator-configuration rules were found in this story's code review, both in field descriptions that had been sitting there through five spec rounds, and the repeated identifier came from the pull request's review**
...
- `src/core/schemas/sealed-run-record.ts:131-141` -- `OracleDisposition.observationIds`, the record's second citation site, quoting AD-33: "every disposition citing supporting observations, and an unsupported disposition invalidating cross-artifact agreement rather than being believed". `resolveOutcome`'s `unsupported-disposition` (`src/core/score/outcome.ts:272-275`) guards the empty list only, so a `violated` disposition citing identifiers that resolve to nothing passed every check in the tree. Complementary rather than overlapping: empty versus dangling.
- `src/core/schemas/evaluator-configuration.ts:54-58` -- the artifact `.meta`, carrying two clauses in one sentence: "Ingest computes this artifact's digest and invalidates the run when it is absent or incomplete." AD-24 states it in the spine's stage-signature rule and AD-11 restates it as "the evaluator configuration digest is computed on ingest from the artifact of AD-24 -- and none of the three proves the caller used what it declared, which AD-32 states as the trust boundary". The recomputation is the only operand in the design that reads an artifact rather than a caller's declaration; `src/core/seal/seal.ts:106` is the precedent for digesting a whole parsed artifact.
...
**The consumers this product must satisfy, and the eight that do not exist yet**
...
- **No rung exists for a dangling citation from either site, a cross-artifact disagreement, an admitted prohibited input, an absent evaluator configuration, an evaluator configuration whose digest does not recompute, or a malformed judge result.** AD-16's title has two clauses, "a prohibited input **or** an unaccounted isolation manifest invalidates a run", and AD-21's Invalid enumeration names only the manifest one, so the row at `:248-258` guarded "an unaccounted isolation manifest under AD-16" is not the prohibited-input rung and is not the evaluator-configuration one either. `OutcomeInputs.judgeConduct` (`src/core/score/outcome.ts:104-119`) is the field that reads the judge condition, and it is per oracle while `JudgeResult` is keyed by `(rubricId, criterionId)`. All seven are recorded with owner and no target, joined by a repeated record identifier for an eighth.
...
Four things moved in the frozen block and nothing already approved was removed or weakened: three matrix rows added, the counts corrected, the last matrix row widened past the quotation audit, and the Always clause's one propagating fault site corrected to two. That last one is the property the review rounds spent the most time pinning, so it is worth stating plainly rather than leaving inside a count: `digestArtifact` canonicalizes the whole evaluator configuration, so the recomputation is a second reachable path to `non-canonicalizable-value` from the same module. The rule itself is unchanged. Ingest raises nothing of its own and catches nothing. The build session recorded these as a divergence list for renegotiation rather than editing the block itself, which was the right instinct; the list is folded into this note and removed, because two records of one amendment drift apart. The consequence for the next story is that it owes eight ladder rows, up from the four this amendment first named, which is recorded in its acceptance criteria.
...
Eight of the eleven have no rung on either shipped ladder; four were known at approval, three were found in this story's code review, and one came from the pull request's review. Forcing them onto `internallyInconsistent` would make the persisted basis read as an AD-17 evidence failure at FAIL when AD-32 and AD-17 each say the condition invalidates. The next story adds the rungs, because it is the story that already opens `ladder.ts`; adding a row here would touch a generated table this story is forbidden to move. Owners: the `operationId` collision, AD-17's rubric half, and the private-artifact digest go to `deferred-work.md`, joined after the code review by the empty-violation-string schema gap, `contractId`'s missing second operand, and the manifest reference digest; the eight rungless conditions go to the next story's acceptance criteria, so they are committed work rather than a deferral. That story also re-points all eight off `null`, and the drift test then asserts the mapping carries no `null` at all, which is what turns a committed intention into something a gate proves.
...
**Decision 10: a repeated record identifier is a condition, and that is what makes the two pass-through orders total.**
The pull request's review is correct and the comment defending the behaviour was worse than the behaviour: it said the tied pair "keeps its presented order because the record supplies nothing else to separate them by", and two findings sharing a `findingId` differ in their content, so the record supplies plenty. `SealedRunRecord` refines exactly one array for uniqueness and refines it on `sequence` rather than on an identifier, so a repeated `observationId`, `findingId`, or `oracleId` all parse. Three fixes were available. Canonicalizing each entry as a tie-break key is a total order and the repository's own tool, and it is rejected here: it makes `non-canonicalizable-value` reachable from a finding's `summary`, so a lone surrogate in free text would throw where today it produces a clean product, which is a real regression bought to order a pathological record. A uniqueness refinement on the two arrays is breaking under AD-11 and this story's Boundaries put a `schemaVersion` move under Ask First. So the third: the repeat is recorded as `duplicate-record-identifier`, carrying its subject, the identifier, and the number of occurrences, with a `null` target like its neighbours. Everything downstream addresses these entries by exactly the identifier that repeated: `verdictBasis` names a finding by it, `FindingConfidence` is keyed by it, `resolveOutcome` reads one disposition per oracle, and `auditQuotation` indexes observations into a `Map` keyed on `observationId`. The observation subject was missing from the first version of this decision and the review that caught it also showed why it is the one that matters: the other two change an order, while a repeated `observationId` decided which body a quotation was checked against and so changed whether a defect finding's evidence was witnessed at all, which is the AD-32 property this stage exists to check. Reporting that case was not enough, and the follow-up review was right that it was disproportionate: two permutations of the record produced different condition sets rather than one set in two orders, so one record was reported as inconsistent and the other was not. `auditQuotation` indexed observations into a value rather than a list, which is the one place its procedure turned an existential into a choice the record does not license. The severity is not instability: a record whose repeated identifier names one observation carrying the quoted text and one not carrying it does witness the quote, and the old index reported an unwitnessed quotation against it in whichever permutation left the wrong observation last. So half of the presentations of a conforming record raised an AD-32 declared-versus-observed inconsistency that was not there. AD-40 states the rule as "at least one observation satisfying the condition", so a repeated identifier names both observations and the `.some` now ranges over both. The condition still fires, doing for `observation` what it does for the other two subjects: reporting that the record cannot address its own entries, without being asked to stand in for a determinism the code can supply directly. `citedObservationIds` stays the deduplicated identifier list, since it names what the finding cited rather than what resolved. So the ambiguity is a finding about the run in the same sense as every other condition here. The sorts then stand on an honest claim: the identifier is a total key on any record that does not carry this condition, and the one input whose product still varies with presentation order is one the run already reports as unaddressable. That claim covers `unwitnessedQuotations` only because `auditQuotation`'s own key was extended to reach it. Its declared key is `(findingId, quoteIndex)`, and two findings sharing an identifier each carrying an unwitnessed quote at index 0 tie on both halves while being perfectly distinguishable: different `quote`, different `channel`, two basis lines that say different things, on a rung that persists them. So that sort now runs to the whole payload as well. `quote` is compared directly rather than through a joined key, because it is free text that may carry any code point including the delimiter; `citedObservationIds` may be joined, since its members are `Identifier`. Recorded before the conditions that name a `findingId` or an `oracleId`, so a reader meets the ambiguity before a condition that relies on it. Known-bad state avoided: a persisted `verdictBasis` line naming a finding identifier that matches two different findings, with nothing in the run saying so.
...
Four sessions reviewed the implemented tree: the two that held five rounds of context on the spec, re-running their own earlier reasoning against the code, and two fresh sessions with none. Twenty-four findings, all closed in this pass; three were routed to `deferred-work.md` with owners because their inputs are genuinely absent. The reviews are why this story ships eleven conditions rather than seven.
...
- `npx vitest run tests/ingest` -- 2 files, 42 tests, green. Twenty-eight cases in `ingest.test.ts` (one per I/O Matrix row, plus the two evaluator-configuration conditions, the disposition citations, the ordering and tie-break cases for all four sorted families, the two purity cases, the rungless-conditions-on-the-product case, and the nothing-else-throws case) and five in `conditions.test.ts`.
- `npm run test:coverage` -- exit 0. `src/core/ingest/**` is at 100% statements and 100% branches (85/85 and 28/28), floor 90/90. Suite-wide: 102 files, 3409 tests, 97.11% statements and 92.69% branches.
