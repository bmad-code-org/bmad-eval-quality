...
The pull request's review is correct and the comment defending the behaviour was worse than the behaviour: it said the tied pair "keeps its presented order because the record supplies nothing else to separate them by", and two findings sharing a `findingId` differ in their content, so the record supplies plenty. `observations` is the only array `SealedRunRecord` refines for uniqueness, so a repeated `findingId` or `oracleId` parses. Three fixes were available. Canonicalizing each entry as a tie-break key is a total order and the repository's own tool, and it is rejected here: it makes `non-canonicalizable-value` reachable from a finding's `summary`, so a lone surrogate in free text would throw where today it produces a clean product, which is a real regression bought to order a pathological record. A uniqueness refinement on the two arrays is breaking under AD-11 and this story's Boundaries put a `schemaVersion` move under Ask First. So the third: the repeat is recorded as `duplicate-record-identifier`, carrying its subject, the identifier, and the number of occurrences, with a `null` target like its neighbours. Everything downstream addresses these entries by exactly the identifier that repeated: `verdictBasis` names a finding by it, `FindingConfidence` is keyed by it, and `resolveOutcome` reads one disposition per oracle. So the ambiguity is a finding about the run in the same sense as every other condition here. The sort then stands on an honest claim: the identifier is a total key on any record that does not carry this condition, and the one input whose product still varies with presentation order is one the run already reports as unaddressable. Recorded before the conditions that name a `findingId` or an `oracleId`, so a reader meets the ambiguity before a condition that relies on it. Known-bad state avoided: a persisted `verdictBasis` line naming a finding identifier that matches two different findings, with nothing in the run saying so.
