...
- `--noUncheckedSideEffectImports` (AC-4): confirmed present and recognized in TS 7.0.2 via `npx tsc --all` ("type: boolean, default: true"); absent from `tsconfig.json` and from `--showConfig`'s printed `compilerOptions` because `--showConfig` omits at-default booleans, not because the flag is unset or unrecognized. `npm run typecheck` is clean, so the one-file `src/` has no unchecked side-effect import to flag. No tsconfig change needed; the AC is satisfied by the TS 7 default, not by an explicit setting.

### Post-merge peer review response (2026-08-10)

An independent Claude Code peer session reviewed the merged PR #6 by execution (not just reading), reproducing every claim it made. Findings: 2 High, 8 Medium, 7 Low. All 2 High and all 8 Medium were fixed on branch `fix/story-1-1-review-findings`, each re-verified by reproducing the original repro against the fixed code:

- **H1 (silent no-op on paths needing percent-encoding):** `import.meta.url === \`file://${process.argv[1]}\`` never matches when the script's path needs percent-encoding (spaces, non-ASCII), so `main()` silently never runs - a licence/age gate switched off, not merely idle, on a green `exit 0`. Fixed in both `audit-lockfile-age.mjs` and `check-licenses.mjs` via `import.meta.url === pathToFileURL(process.argv[1]).href`. Re-reproduced the peer's exact repro (a `dir with space/` copy) against the fix: correctly exits 1 with the violation.
- **H2 (no canary for the licence gate):** Added `scripts/fixtures/licence-canary/` (a GPL-3.0 entry nested behind a scoped parent, chosen specifically to double as M1's regression lock) and a `canary-licence` job in `pr-checks.yml` asserting nonzero exit, the exact violating package, and the exact dependency path.
- **M1 (`ancestorScopesOf` wrong for scoped packages):** rewrote to split on `/node_modules/` boundaries instead of striding path segments by two, which undershot every scoped ancestor and never reached the root scope. Re-verified against all 8 real `@biomejs/cli-*` platform binaries (the exact case AD-25 names) - each now resolves a real `eval-quality > @biomejs/biome > @biomejs/cli-*` chain instead of falling back to the raw lockfile key.
- **M2 (`--window-days` unvalidated -> NaN -> whole audit passes):** `auditLockfileAge` now throws on a non-finite/negative `windowDays` instead of building an `Invalid Date` cutoff that no entry can ever compare "young" against.
- **M3 (unparseable registry timestamp treated as OLD):** an unparseable `publishedAt` is now added to `unfetchableEntries` (fails closed) instead of silently comparing false against the cutoff.
- **M4 (aliased dependency audited against the wrong package):** `collectLockedEntries` now prefers `meta.name` (present on `npm:pkg@x` alias entries) over the path-derived name, so the registry request targets the package actually installed.
- **M5 (canary-age can't distinguish its own reason from a registry outage):** the unfetchable-metadata branch prints the byte-identical `zod@4.4.3 (node_modules/zod)` shape as the young-entry branch, so a bare `grep -q "zod@4.4.3"` passes on either. The canary now greps for the age-specific line and the "published" detail line together. Re-verified: stubbing `fetch` to throw makes the old grep pattern match (false pass) and the new one correctly not match.
- **M6 (neither gate cross-checks `resolved` against the real registry):** both scripts now fail closed on any non-link entry whose `resolved` does not start with `https://registry.npmjs.org/`, so a lockfile edit that relabels a package's licence/version while repointing `resolved` elsewhere can no longer validate the wrong artifact.
- **M7 (actions pinned to mutable, inconsistent tags):** `actions/checkout`, `actions/setup-node`, `actions/setup-python` were a live `@v7`/`@v4` mix (dependabot bumped some occurrences after PR #6 merged; the story's own new occurrences were untouched). All pinned to the same commit SHA per version, everywhere (`pr-checks.yml`, `publish.yml`, `gitleaks-check.yml`).
- **M8 (`prepublishOnly` claimed to "close" the local-publish bypass; it doesn't):** `npm publish --ignore-scripts` skips `prepublishOnly` outright, so the true control is credential-level, not a script. `publish.yml` now uses npm trusted publishing (OIDC `id-token: write`, `npm publish --provenance`, no `NODE_AUTH_TOKEN`/`NPM_TOKEN`) instead of a static token - nobody holds a long-lived publish credential to bypass in the first place. Requires a one-time npm-side Trusted Publisher link (documented in `publish.yml`'s header comment) before any real publish can succeed; harmless until then since AD-18 already blocks every run.
- **L2/L3 (no fetch timeout; retries non-retryable 4xx):** added `AbortSignal.timeout(15_000)` to the registry fetch and a `NonRetryableFetchError` that skips the retry loop for any 4xx other than 429.
- **L4 (copy-paste drift across 6/3 duplicated blocks, already inconsistent):** extracted `.github/actions/assert-npm-version` and `.github/actions/audit-lockfile-age` composite actions; every job (`pr-checks.yml` and `publish.yml`) now references them instead of inline copies.
- **L5 (`check-licenses.mjs` scanned `link: true` workspace-symlink entries; `audit-lockfile-age.mjs` already excluded them):** added the same exclusion to `check-licenses.mjs`.
- **L6 (canaries only prove the policy is live, not that the copied `.npmrc` is complete):** `canary-git`/`canary-remote` now assert the copied `.npmrc` contains all four policy lines before running `npm ci`.
...
**Post-merge review-response changes (branch `fix/story-1-1-review-findings`):**

- `scripts/audit-lockfile-age.mjs` (H1, M2, M3, M4, M6, L2, L3 fixes: entry-point URL comparison, `windowDays` validation, unparseable-timestamp handling, alias-aware name lookup, off-registry `resolved` check, fetch timeout, non-retryable-4xx handling)
- `scripts/check-licenses.mjs` (H1, M1, M6, L5 fixes: entry-point URL comparison, scoped-package `ancestorScopesOf` rewrite, off-registry `resolved` check, exclude `link: true` entries)
- `scripts/fixtures/licence-canary/package.json`, `scripts/fixtures/licence-canary/package-lock.json` (new, H2 - not npm-installable by design, `check-licenses.mjs` needs no install)
- `.github/actions/assert-npm-version/action.yml` (new, L4)
- `.github/actions/audit-lockfile-age/action.yml` (new, L4)
- `.github/workflows/pr-checks.yml` (M5 grep fix, L6 `.npmrc` completeness assertion, new `canary-licence` job, job-level `timeout-minutes`, M7 SHA-pinned actions, L4 composite-action calls)
- `.github/workflows/publish.yml` (M7 SHA-pinned actions, M8 trusted-publishing/OIDC/provenance instead of `NPM_TOKEN`, `github.repository` guard, L4 composite-action calls, job-level `timeout-minutes`)
- `.github/workflows/gitleaks-check.yml` (M7 SHA-pinned `actions/checkout`)
