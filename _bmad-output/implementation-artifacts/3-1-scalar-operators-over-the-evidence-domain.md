...
571). `core/canonical/` is direct precedent: it shipped in Epic 1 and is not in the printed tree
...
```
This lives in `core/evaluate/`, apart from `core/schemas/primitives.ts` where `JsonValue` is defined.
`ABSENT` never appears on the wire: it is never parsed, never serialized, never part of a published
schema, so it does not belong in the module whose job is the wire-format value domain. This mirrors
Story 2.3's own reasoning for keeping `StructuralFailure` beside the registry it serves. Story 2.3
considered placing it beside the stage that first throws it and rejected that option.
...
**The absent-collection-typed boundary belongs to Story 3.2. These functions cannot police it
themselves.** AD-4: *"A pointer the declared response descriptor types as a
...
- `covers-by-key`, AD-4's eleventh operator (Story 3.3's own title and scope). The opening sentence
  ...
  and does not restate epics.md's own "eleven" verbatim.
- The three connectives (`all`, `any`, `not`), the two quantifiers (`for-all`, `for-any`), and AD-4's
  ...
  and its propagation rules) are all Story 3.2's ("Connectives, quantifiers, and three-valued resolution").
...
- `CheckResolution` tree assembly (`src/core/schemas/evidence-artifact.ts`): that shape's own comment
  ...
  Story 3.2's orchestration. **Read `CheckResolution`'s shape** before writing
...
1. **`src/core/schemas/scoring-policy.ts` has no budget field.** Add one, last among the schema's
   ...
   `required` array is the fastest way to confirm the count directly; do not trust this
   sentence.)
   ```ts
   regexMatchStepBudget: z
     ...
     .describe(
       ...
       "construction rather than escalating: the budget bounds a pure, static, pre-execution " +
       "complexity estimate, not a literal engine-internal step count. The published default " +
       "artifact carries 10000.",
     ),
   ```
   ...
   count with a stale prose restatement) twice; do not repeat it here. **Revised twice after this
   story's own review rounds, exactly the gap the sentence above warns against; kept here as the
   visible record:** round one dropped the dangling "(see the regex operator's own
   Decision below)" pointer, since that text ships into the published, externally-facing schema where
   "Decision below" resolves to nothing; round two raised `10000` to `1000000` and added a sentence
   naming that the observed string's length is controlled by the caller; the author has no control
   over it. The shipped
   `.describe()` text in `src/core/schemas/scoring-policy.ts` is the current, correct version; read it
   there; do not trust this block.
2. **`src/core/schemas/faults.ts`'s `RuntimeFaultCode` union carries only two members**
   ...
   is exactly such a budget, so **reuse it: do not mint a new code for the budget itself**: edit
   the AD-28 table row's `Commanded by` column from `AD-1, AD-19` to `AD-1, AD-4, AD-19`
   (`ARCHITECTURE-SPINE.md:414`): a citation-accuracy edit. It does not add a new fault class, so it
   needs no new
   ...
   exact phrase to `oracle-error`, a score-side **outcome state** (AD-33's territory, Epic 6). That is
   not a runtime fault, and this story's own AC 1 forbids Epic 3 from mapping onto outcome states. Citing
...
3. **Locate every existing `ScoringPolicy`-constructing fixture and add the new required field: verify
   the actual impact before editing, it is narrower than it looks.** `grep -rln "ScoringPolicy"
   ...
   mechanism; it is not assumed: `tests/schemas/artifact-reject-fixtures.test.ts`'s scoring-policy reject
   ...
   asserts `mutants.length` is `toBeGreaterThan(20)` (around lines 115–117), a floor never pinned to an
   exact count; `tests/schemas/artifact-registry.test.ts` is a bare key list. The one edit beyond the
   fixture itself: regenerate `schemas/scoring-policy.schema.json` (`npm run generate:schemas`,
   `npm run check:schemas`), and re-measure `tests/schemas/published/keyword-mutation.test.ts`'s pinned
   `'scoring-policy': 29` census directly by re-running the count: do not guess the new number, Story
   2.3's own Completion Notes record exactly this discipline after its own single-field schema addition.
...
Story 3.2's resolution layer invokes them generically. A per-function-different arity would cost it
a dispatch table; a uniform arity costs it one calling convention.
...
- **`deepEquality(a, b, artifactPath): boolean`**. *"structural over canonical JSON per AD-27, never
  ...
  **`equality`'s scalar branch looks like an optimization. It is not one: it keeps this operator's fault
  surface where it already was.** This design point moved three times during story creation, and all
  three are recorded because a future reader would plausibly re-derive the first two before finding the
  third (Decision 2).
  ...
  - **Draft 3:** delegate unconditionally to `digestArtifact` for every operand, scalar or compound,
    ...
    ordinary API output. They are not a pathological case), a lone UTF-16 surrogate in free text
    ...
    scalar output invalidates the run. It is not caught as a defect. Same failure, different
  ...
  **`deepEquality`'s fault surface, unconditional and unchanged from this story's earlier drafts:**
  ...
  genuinely cannot be canonicalized is unevaluable: calling it merely "not equal" would misdescribe the
  case. `RuntimeFault` is the correct signal per the Errors convention (a fault, never a finding).
  `equality` shares this same fault surface, but **only** when it falls through to the compound branch;
  never on a scalar comparison.

### AC 4 — Membership-family operators: `containment`, `setMembership`

Both use structural (canonical-JSON) element matching. Neither uses `===`, for the same reason
`deep-equality` does: a set or a container's elements are not guaranteed scalar.
...
- **Pattern validity is checked here. It is not assumed.** `AnchoredPattern` (`expression.ts:132-139`)
  checks only the pattern's first and last character (its own comment says so explicitly, lines
  112-119, `ANCHORING_RESIDUAL`), so a schema-valid pattern can still be an invalid ECMA-262 source
  ...
  It is not this function's job: this function only guards against a source that fails to *construct* at
  all, never against a constructible-but-forbidden dialect feature.
...
- **The match-step budget is a pure, deterministic, pre-execution static gate in two tiers. It is not a
  ...
  documented, deliberately imperfect check. It is not a full parser, and its accepted residuals are
  named openly. **Only the first of the three residuals below is safe-direction; the "conservative"
  ...
  2. The character-class strip is itself approximate (a class containing an escaped `]`, e.g. `[\]+]`,
     ...
     imperfection well short of a full parse. **This one is not safe-direction on its own**: a review round
     ...
     specific case now computes the exact correct estimate (50); it no longer produces an inflated
     conservative one.
  3. **A third residual, documentation-only, added by the same review round: a quantified overlapping
     ...
     parser, which this gate deliberately is not. Accepted as a stated, known gap. Chasing it with
     a heuristic would only trade a false negative here for false positives on ordinary patterns
     elsewhere.
...
- **`ordering(collection, key, order, artifactPath): boolean`** where `order: 'ascending' | 'descending'`.
  ...
  empty-collection wrapper handles the zero-element case; this function does not). For each adjacent
  pair `(el[i], el[i+1])`: both must be plain objects carrying `key` as an own key (per `expression.ts`'s
  ...
  locale-aware**: chosen for determinism. It is not because observed evidence is guaranteed to carry any
  ...
  already picked, for consistency. It does not invent a second one. It happens to sort matching-
  ...
  this project defines. It does not govern an arbitrary system under test's response bodies, and RFC
  3339 itself admits
...
1. **`equality`/`deep-equality`:** reuse `gateCContract`'s real usage where it exists: O-001's
   ...
   - **`deep-equality` only:** an unsafe-range integer (e.g. a response body carrying
     ...
     never returning `false`: the realistic trigger from Draft 3's rejection.
...
3. **`setMembership`:** reuse `gateCContract`'s two real nodes: O-002's literal-array set
   ...
   structural (non-scalar) set-member case proving canonical-JSON matching, never `===`.
4. **`regexMatch`:** an ordinary anchored pattern against ordinary input (accept and reject); a
   ...
   `'operator-cannot-accept-operand'`, never a raw `SyntaxError`; a nested-quantifier pattern (e.g.
...
7. **`shape`:** reuse `gateCContract` O-005's descriptor
   ...
   descriptor (a required key absent from `permittedKeys`) proving it is unsatisfiable, never
...
- [x] [Review][Patch] LOW. **No fixture proved `existence`/`absence` read `=== ABSENT`, never JS
...
- [x] [Review][Patch] MEDIUM. **The AD-28 table's `operator-cannot-accept-operand` row cited `AD-4,
      ...
      Epic 6). That is not a runtime fault; this story's own AC 1 forbids Epic 3 from mapping onto
      outcome states.** Fixed: the row now cites `AD-4` alone; AC 2 point 2's prose and Task 2's own
      subtask text updated to match and to record the correction. `npm run check:docs`,
      `npm run lint:spine`, `npm run build:shareable` re-run; `npm run check:shareable` confirmed green.

A second, independent fresh-session review of the committed diff, run against the running code with
direct timing reproduction, never reasoning alone. Four findings, all fixed below.
...
- [x] [Review][Patch] **BLOCKING.** **`CHARACTER_CLASS_CONTENTS` ran on the raw pattern, before escape
      ...
      Reproduced directly, under a hard subprocess timeout, never trusted from the report: the
      ...
      inside it, and the separate `quantifierCountSource` line in `regexMatch`, leaving one escape pass
      where there had been three. Two fixtures added to `tests/evaluate/operators.test.ts`:
      `^\[(a+)+\]$` now throws
      `budget-exhausted` (the previously-hidden shape is caught); `^[a\]b+]$` at length 50 under budget
      60 does not throw, and, verified directly, never merely assumed, now resolves the exact correct
      estimate (50, since the pattern is a single zero-quantifier character class once the escaped `]`
      no longer confuses the boundary). The estimate is no longer the previously-documented inflated
      conservative one (100). Every fixture from the prior two rounds re-run and confirmed unchanged (68
      tests, none moved), matching the finding's own "prototyped and verified" claim.
- [x] [Review][Patch] MEDIUM. **Documentation gap, never a code bug, correctly flagged as such: a
      ...
      now names three residuals, up from two, explicitly scoping the "safe direction" claim to only
...
1. **This story builds ten operators, never the "eleven" its own opening sentence could be misread as
...
2. **`equality` is a three-way, cost-ordered branch: type mismatch, then same-scalar `===`, then
   ...
   - **Draft 2: `===` for scalars, throw `RuntimeFault('operator-cannot-accept-operand', …)` on two
     ...
     whether it fires is decided by what the system under test actually returns; the contract itself
     never decides it. A misbehaving SUT returning a compound value where a scalar was expected throws
     and invalidates the run; it does not resolve the comparison. This is
     the exact failure AD-26's own
   ...
   - **Draft 3: delegate unconditionally to `digestArtifact`, both operators, every operand.** Closes
     ...
     `false`: the same "detected defect becomes broken measurement" failure as Draft 2, this time
     entered through the value domain, where Draft 2's was operand shape.
...
3. **`containment`'s dual-mode construction is a flagged, explicit judgment call, never a certainty**:
   ...
   which does not contradict the dual-mode reading. It simply leaves it unaddressed: the gloss is a
   ...
   that is new evidence for revising this one function. It is not grounds to treat the rest of the story
   as suspect. **Consequence:** flagged explicitly for the peer review round this story's creation workflow
   ...
   documented, tested boundary, never a silent surprise.
4. **The regex match-step budget is a two-tier static gate: structural rejection of nested quantifiers,
   ...
   check that names its accepted residuals (AC 5 names three, only one of them
   ...
   nested-quantifier scan, and the default budget value (1,000,000, raised from an initial 10,000 after
   a second review round found the smaller value fault-prone against ordinary-length evidence) are this
   story's own construction, recorded here because no spine text states them.
   At 1,000,000 the linear tier is close to inert for realistic evidence: a single-quantifier pattern
   now needs roughly 500,000 characters to trip it. The structural nested-quantifier rejection above
   is the gate's real backstop; the linear tier is now mostly a runaway guard against pattern/length
   combinations far past anything a real contract's evidence would carry. Recorded here so a future
   reader does not retune the default back down thinking the
   linear tier does more day-to-day work than it does.
...
7. **Every operator function's last parameter is `artifactPath: string`, uniformly, even on functions
   that never throw. The unused ones are underscore-prefixed.** A per-function-different arity would
...
8. **Absent-collection-typed pointers are Story 3.2's boundary to police, never this story's operators'
   ...
   **Consequence:** every operator taking a possibly-collection operand documents its own `ABSENT`
   handling as correct *only* for the non-collection-typed case (AC 4, AC 6). Story 3.2's resolution
...
9. **`RuntimeFaultCode` becomes data-derived (`RUNTIME_FAULT_CODES`), mirroring `FAILURE_CODES`'s own
   ...
   citation, the new `operator-cannot-accept-operand` code); the cheap, local fix is taken now, closing
   what would otherwise become a second stale-abstraction gap. **Consequence:** no automated AD-28-vs-`faults.ts`
   cross-check is added (there is no `check:ad28-registry` script, unlike AD-5's
   `scripts/check-ad5-registry.ts`). Building one is real, separate scope, a new CI script rather than an
   ...
   check) has nothing to check against yet.** Kept as the array form: the same order-of-operations
   already shipped for `FAILURE_CODES` (data-derived ahead of `check-ad5-registry.ts`
   existing, per Story 1.5), so this is the established shape for a code registry in this codebase, not
   a premature abstraction invented here. The tradeoff is named explicitly here.
10. **Module placement (`core/evaluate/`, not `core/score/` or `core/compile/`) rests primarily on the
    ...
    ordered by strength: the map first, the epic's own "stops at resolution" boundary second, and the "No
    epic touches `score`" sentence named explicitly as corroborating, not load-bearing.
...
7. `ARCHITECTURE-SPINE.md`: AD-4 in full (lines 185–201, especially the paragraph naming `covers-by-key`,
   ...
   Decision 10 checks against directly).
...
- `src/core/schemas/scoring-policy.ts` gains one field; `src/core/schemas/faults.ts`'s
  ...
  `operator-cannot-accept-operand`, Decision 9). These are the only two `core/schemas/` edits this story
  makes, both additive.
...
- **The two-tier gate's escape-handling order (neutralize backslash escapes, then strip character
  classes) is load-bearing, not stylistic.** Getting that order backward, character-class-stripping
  the raw pattern before any escape-awareness exists, is an actual ReDoS bypass, not a cosmetic
  imprecision:
...
- `tests/schemas/fixtures/artifact-fixtures.ts` (edited: `scoringPolicyFixture` gains
  `regexMatchStepBudget`, `1000000` as of the second review round)
...
- Always structural, with no scalar fast path unlike `equality`.
