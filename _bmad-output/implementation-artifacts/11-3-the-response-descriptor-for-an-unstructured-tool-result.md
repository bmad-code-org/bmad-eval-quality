...
- `src/core/compile/reachability.ts:457-514` -- `descendThroughDescriptor`, the one descent rule, reached from whichever channel the operation nominates. `evaluateReachabilityAgainstOperation` (`:515-637`) is what reaches it and what tests the channel first: `:583-592` is the command branch refusing the three HTTP channels, which is the shape Story 11.4 mirrors for `mcp`, and `:632-634` is the residue that returns reachable for `response-headers`, `response-status`, and `exit-code`, so it is the line the new branch has to precede. `checkExpressionLegChannel` (`:307-358`) is a second site and is easy to miss: `:314-316` builds a witness leg's carried channels from `isCommandOperation` alone, so an `mcp` operation is told its leg carries `response-headers`.
...
Two constraints ride with it. The fix is a repository-wide sweep rather than one line, since two files in one directory already disagree and that is a family. And each corrected sentence is phrased so that Story 11.5 changes a numeral and nothing else when it opens `mcp`, the same discipline the epic sets for the committed-chain count. Any sentence that cannot be phrased that way is named for Story 11.5 in that change's own record.
