...
**Round 3, 2026-09-03 — independent peer review (a separate Claude Code session, `bmad-code-review`, over `git diff main...epic-8-story-3` against the spec).** One `high`, two `medium`, three `low` real findings, all applied; one `low` finding evaluated and declined as out of scope; one `low` finding independently re-confirmed as already correctly deferred.

- **`patch` (high)** — `emit()` built and returned a fully mutable `EvidenceArtifact`, never calling `freezeArtifact`. AD-29 requires a stage to freeze the artifact it owns; `seal.ts` and `preflight/reduce.ts` — the other two `lineage: 'mints'` stages — both do. Fixed: `finalize()` now returns `freezeArtifact(artifact)`; `tests/emit/emit.test.ts` gained `Object.isFrozen` assertions on the artifact, `strength`, and `outcomes`, matching `tests/application/seal.test.ts`/`preflight.test.ts`'s own precedent.
- **`patch` (medium)** — `runId`, read with the identical fallback posture as `mode`/`evaluatorRecommendation` (Boundaries, above), was never compared across trials the way those two are: `trialSetDisagreementsOf` (`score.ts`) checked only `mode` and `evaluatorRecommendation`, so a caller batching trials from two different runs into one trial set went undetected. Fixed: `trialSetDisagreementsOf` now compares `runId` too, firing the existing `trial-set-field-disagreement` Invalid row on a mismatch — no new AD-21/AD-31/AD-33 registry entry, since the row already exists and its basis-line producer is generic. A new matrix-adjacent test in `tests/score/score.test.ts` covers it.
- **`patch` (medium)** — `score()`'s own `disposition`-default (the ambiguity-guard's `'not-attempted'`) and `uncitedFindings` (the broader, non-`defect`-only sibling of `uncitedDefectFindingGaps`) were only exercised through `result.ladder`/`.assessment`, never through `result.outcomes`/`result.uncitedFindings` themselves. Added an assertion on the existing ambiguity-guard test, plus a new case distinguishing `uncitedFindings` from `uncitedDefectFindings` with a non-`defect` finding.
- **`patch` (low)** — `tests/emit/emit.test.ts` never exercised non-empty `uncitedFindings`/`coverageGaps` passthrough, nor `comparabilityKey`/`strength.vector` under the zero-admitted-probe scenario (Matrix row 3's rejected-probe case). Added both.
- **`patch` (low)** — two more sentences carried the same stale "module: null"/"not built" framing this story's own Boundaries text already committed to correcting everywhere it appears: `src/core/schemas/evidence-artifact.ts`'s `.meta().description` ("it has no caller yet because `score` and `emit` both carry `module: null`" — false on both counts, and `emit` is now `checkModeAgreement`'s one caller) and `docs/explanation/roadmap.md` (a "Not built" section devoted entirely to `emit`, and a "Next" section still promising to "give `score` and `emit` a module"). Both corrected; `evidence-artifact.schema.json` regenerated via `npm run generate:schemas` to match.
- **Declined (low)** — `ScoredOutcomesAndVerdict.uncitedFindings` is typed `readonly string[]`, not `readonly FindingId[]` as the frozen Boundaries text states. Real drift, left as `string[]`: no `core/schemas` module exports a bare `FindingId` TypeScript type for any of the seven sibling AD-27 identifier primitives (`BehaviorId`/`OracleId`/`ProbeId`/`WaiverId`/`DefectId`/`FindingId`/`RubricId` all export only their Zod schema, confirmed by direct read of `primitives.ts`), so introducing one bare export for `FindingId` alone would be a new, story-local convention with no precedent anywhere else in this codebase. Noted here as the correction to the frozen Boundaries text rather than a code change.
- **Re-confirmed (low)** — the peer independently raised the `qualifiedProbe` fixture's schema-invalid `probeId` and, on its own re-check, found the fixture is asserted against by literal string in roughly ten places across `tests/score/qualification.test.ts` and `tests/score/witness.test.ts`, neither of which this story otherwise touches — confirming Round 2's `defer` disposition (above) rather than the "trivial, same-file" framing the peer's own first pass proposed before checking.

**Round 4, 2026-09-03 — CodeRabbit, over the pull request.** Two real findings, both applied; this round also corrects Decision 12.

- **`patch` (high, CodeRabbit's own "resolve before merge" flag)** — `emit()` assembles its return value under one unsafe cast (`scored.ladder.verdict as Verdict`, Decision 12) and had no runtime check that the assembled object actually matches `EvidenceArtifact` before freezing it, unlike `seal.ts`'s own `validateAssembledBrief`, which `.safeParse()`s its own assembled value first. A `null` verdict reaching `emit()` despite the caller's precondition would mint and freeze a schema-invalid artifact. Fixed: `finalize()` now runs `validateAssembledArtifact` (a `safeParse`-then-`TypeError` check, duplicating `seal.ts`'s own `dotPath`/error-shape rather than sharing it, since neither is exported) before `freezeArtifact`. **This corrects Decision 12**, which read "the frozen Boundaries already commits to exactly one throw in this stage" as forbidding a second one; re-read now, both throws are the same *kind* (a precondition-violation check unreachable through any domain input this module's own construction takes), so a second instance of that kind is not a second kind. `tests/emit/emit.test.ts` gained a case pinning the null-verdict path to this new throw, plus a nested-path formatting case mirroring `tests/seal/seal.test.ts`'s own precedent for the duplicated formatter.
- **`patch` (low)** — `README.md`'s `generate:worked-example` description omitted `emit` from the functions the script runs, and its lineage-producer list (`src/core/seal/seal.ts`, `src/core/preflight/reduce.ts`) omitted `src/core/emit/emit.ts` now that it is a third real lineage writer. Both corrected; `_bmad-output/shareable/eval-quality-readme.html` regenerated via `npm run build:shareable` to match.
...
**Decision 12: `emit.ts` trusts, rather than re-derives, a `scored.ladder.verdict` of `null` — but validates the artifact it assembled from that trust before freezing it (amended, Round 4).** `LadderResolution.verdict` is `Verdict | null`, `null` being AD-21's Invalid rung, and neither the frozen Boundaries nor the I/O Matrix names a row for `emit` receiving one — the closest precedent is `scripts/worked-example-target.ts`'s own `if (ladder.verdict === null) fail(...)` guard, which sits in the calling script immediately before its former hand-assembly and stays there unchanged by this story's diff (Code Map only touches the object-literal lines after it). AD-21 states an Invalid run "never becomes a contract verdict," which reads as the caller's obligation to stop before minting an artifact at all, not a case `emit` re-derives a value for: `emit.ts` still reads `scored.ladder.verdict as Verdict`, trusting the precondition the same way `seal()`'s own null-direction guard trusts one call earlier in the pipeline, with the comment at the cast's own site carrying this reasoning. What Round 1's text got wrong was reading "the frozen Boundaries already commits to exactly one throw in this stage" as forbidding a second throw statement; CodeRabbit's Round 4 finding is that the assembled artifact was never schema-validated before freezing, unlike `seal.ts`'s own `validateAssembledBrief` precedent for the identical shape (an unsafe cast, one call earlier, that TypeScript cannot actually guarantee). `finalize()` now calls a local `validateAssembledArtifact` (mirroring `seal.ts`'s `safeParse`-then-`TypeError` shape) before `freezeArtifact`. This is not a second *kind* of throw: both the mode-agreement check and the schema validation are precondition-violation checks unreachable through any domain input this module's own construction takes, so the Boundaries text held on substance even though its "exactly one throw" phrasing needed sharpening.
