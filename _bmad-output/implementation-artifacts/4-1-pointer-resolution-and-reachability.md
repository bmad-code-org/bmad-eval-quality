...
**Rejected (by-design or out of scope), and one deferred:**

- The root-collection index carve-out never validates the literal index against the declared
  collection's own `expectedCardinality`. Real gap, but not this story's declared scope (AC 1 never
  names collection-cardinality reachability, and AC 6 fixture 38 only proves the carve-out fires when
  declared, not that it bounds the index). Deferred to `deferred-work.md`, filed against Story 4.2's
  adjacent `quantifier-over-non-collection` territory.
