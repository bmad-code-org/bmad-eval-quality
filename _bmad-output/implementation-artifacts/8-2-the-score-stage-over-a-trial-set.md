...
**Round 5, CodeRabbit review of the pushed PR, 2026-09-03.** Two more gaps in `reduce-trials.ts`, both in the same validation this story added the module's real caller specifically to close, and both a level below the earlier rounds' ambiguous-match class: not a missing check, but a check written in a form a coerced or prototype-derived input still slips past. `TRIAL_VOTE_STATE_OF[vote.state] === undefined` (Rounds 1-4's own form of the out-of-domain-state guard) is fooled by any state string naming an inherited `Object.prototype` member (`"toString"`, `"constructor"`, `"hasOwnProperty"`, `"valueOf"`): the plain object built by `Object.fromEntries` inherits the prototype, so the lookup resolves to the inherited function rather than `undefined`, clearing the guard and voting the bogus state in. Replaced with `Object.hasOwn(TRIAL_VOTE_STATE_OF, vote.state)`, checked before the lookup runs at all. Separately, `!(catchThreshold >= 0 && catchThreshold <= 1)` uses `>=`/`<=`'s own operand coercion, so `null` (coerces to `0`), a numeric string (coerces via `ToNumber`), and a boxed `Number` (coerces via its own `valueOf`) all satisfy the range check without ever being the primitive `number` it means to bound. A `typeof catchThreshold === 'number' && Number.isFinite(catchThreshold)` guard now runs first and rejects all three before the coercing comparison is reached.
