...
- source_spec: `_bmad-output/implementation-artifacts/4-1-pointer-resolution-and-reachability.md`
  summary: `evaluatePointerReachability`'s root-declared-collection carve-out admits any canonical array-index token, never checking it against the declared collection's own `expectedCardinality` (`exact`/`at-most`/`page-bounded`), so a literal index provably out of bounds still passes reachability.
  evidence: `CollectionLocation.expectedCardinality` (`interface.ts:32`) already declares the bound; `evaluatePointerReachability`'s `rootIsCollection === true && ARRAY_INDEX_PATTERN.test(firstToken)` branch (`reachability.ts`) returns reachable on the index grammar alone. This is the same class of gap Decision 8 fixes for `stdout`/`stderr` (a pointer provably always `ABSENT` under any conforming resolver should fail reachability, not merely go unchecked), applied here to a numeric bound instead of a channel type. Not this story's declared scope (AC 1 never names collection-cardinality reachability, and this story's own AC 6 fixture 38 only proves the carve-out fires when declared, not that it validates the index against the declared count), and closely adjacent to Story 4.2's `quantifier-over-non-collection` territory, so it belongs with that story rather than reopening this one.
