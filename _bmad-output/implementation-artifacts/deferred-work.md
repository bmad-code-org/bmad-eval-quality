...
The last entry, an environment-key gap epic 9's third adversarial review opened, closed in the epic
11 tail pass: `CommandTargetAuthorization.permittedEnvironmentKeys` bounds the keys a call may
carry, `command-line-adapter.ts` refuses an unnamed key before any process spawns, and the `cli`
conformance arm's tenth assertion, `command/deny-unauthorized-environment-key`, pins that refusal at
zero underlying calls. The reasoning lives in that field's own description, in the adapter's
`buildEnv` comment, and in the `CHANGELOG.md` entry that discloses the count moving from 15 to 16.
...
the rule above asks for when an entry is deleted. The one item that outlived that
sweep, the environment-key gap epic 9's own third adversarial review opened after
it had run, closed in the epic 11 tail pass and its pointer is at the top of this
file.
